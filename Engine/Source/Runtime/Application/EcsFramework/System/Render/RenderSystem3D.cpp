...
#include "Runtime/Application/EcsFramework/System/Render/RenderSystem3D.h"
...
namespace JEngine
{
	...
	struct Subfrustum
	{
		float subfrustum_near;
		float subfrustum_far;
		float circumsphere_r;
		glm::vec3 circumsphere_center;
		glm::vec3 points[8];
	};
	...
	namespace Utils
	{
		...
		static glm::mat4 getLightSpaceMatrix(const float nearPlane, const float farPlane, float cameraFOV, float aspect, const glm::vec3& lightDir, const glm::mat4& view)
		{
			...
			center /= corners.size();
			glm::vec3 point = glm::vec3(corners.back());
			float sphere_radius = (center - point).length();
	...
	void RenderSystem3D::OnUpdateEditor(Timestep ts, EditorCamera& camera)
	{
		...
		defaultShader->SetFloat("based_ambient", EnvironmentSystem::environmentSettings.basedAmbient);
		...
		// Directional light
		{
			...
			for (auto e : view)
			{
				...
				// draw debug geometry
				if ((int)e == ConfigManager::selectedEntity && entity.HasComponent<DirectionalLightComponent>())
				{
					auto& geometry = entity.GetComponent<DirectionalLightComponent>().mGeometry;
					if(geometry)
					{
						glm::mat4 transforms = glm::mat4(1.0);
						transforms = glm::translate(transforms, glm::vec3(0.0, -2.0, -20.0));
						transforms = transforms * transform.GetRotationMatrix();
						Renderer3D::DrawGeometry(transforms, camera.GetProjection(), geometry, (int)e);
					}
				}
