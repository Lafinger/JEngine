...
namespace JEngine 
{
	...
	void EditorCamera::OnUpdate(Timestep ts)
	{
		...
		else if (Input::IsMouseButtonPressed(Mouse::ButtonRight))
		{
			...
			if (Input::IsKeyPressed(JENGINE_KEY_A))
				mFocalPoint -= GetRightDirection() * mCameraSpeed * 0.05f;
			if (Input::IsKeyPressed(JENGINE_KEY_D))
				mFocalPoint += GetRightDirection() * mCameraSpeed * 0.05f;
			if (Input::IsKeyPressed(JENGINE_KEY_W))
				mFocalPoint += GetForwardDirection() * mCameraSpeed * 0.05f;
			if (Input::IsKeyPressed(JENGINE_KEY_S))
				mFocalPoint -= GetForwardDirection() * mCameraSpeed * 0.05f;
			if (Input::IsKeyPressed(JENGINE_KEY_Q))
				mFocalPoint -= glm::vec3(0.0f, 1.0f, 0.0f) * mCameraSpeed * 0.05f;
			if (Input::IsKeyPressed(JENGINE_KEY_E))
				mFocalPoint += glm::vec3(0.0f, 1.0f, 0.0f) * mCameraSpeed * 0.05f;
	...
	void EditorCamera::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseScrolledEvent>(JENGINE_BIND_EVENT_FN(EditorCamera::OnMouseScroll));
	}
