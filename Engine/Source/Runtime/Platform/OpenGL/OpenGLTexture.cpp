...
namespace JEngine
{
        ...
        : mPath(path)
    {
        ...
        if (data)
        {
            ...
            JENGINE_CORE_ASSERT(internalFormat & dataFormat, "Format not supported!");
        ...
        {
            JENGINE_CORE_ERROR("Load texture failed!");
    ...
    void OpenGLTexture2D::SetData(void* data, uint32_t size)
    {
        ...
        JENGINE_CORE_ASSERT(size == mWidth * mHeight * bpp, "Data must be entire texture!");
    ...
    void OpenGLCubeMapTexture::Generate()
    {
        ...
        for (unsigned int i = 0; i < mPaths.size(); i++)
        {
            ...
            {
                JENGINE_CORE_ERROR("Cubemap don't loaded correctly!");
                stbi_image_free(data);
            }
        }
