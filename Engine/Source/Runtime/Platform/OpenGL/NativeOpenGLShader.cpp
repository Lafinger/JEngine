...
namespace JEngine
{
	...
	{
		GLint location = glGetUniformLocation(mRendererID, name.c_str());
		//JENGINE_CORE_ASSERT(location != -1);
		glUniform1i(location, value);
	}
	...
	std::unordered_map<GLenum, std::string> NativeOpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			JENGINE_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void NativeOpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				JENGINE_CORE_ERROR("{0}", infoLog.data());
				JENGINE_CORE_ASSERT(false, "Shader compilation failure!")
					break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			JENGINE_CORE_ERROR("{0}", infoLog.data());
			JENGINE_CORE_ASSERT(false, "NativeOpenGLShader link failure!")
				return;
		}
