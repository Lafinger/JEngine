...
namespace JEngine
{
    ...
    void SceneHierarchyPanel::OnImGuiRender(bool* pOpen, bool* pOpenProperties)
    {
			...
			if (mContext)
			{
				...
				if (ImGui::BeginPopupContextWindow(0, 1, false))
				{
					...
					if (ImGui::MenuItem("Create Directional Light"))
					{
						...
						auto& geometry = entity.GetComponent<DirectionalLightComponent>().mGeometry;
						geometry = CreateRef<Geometry>(GeometryType::Light);
						auto& rotation = entity.GetComponent<TransformComponent>().Rotation;
						rotation = glm::vec3(0.01,1.0,0.01);
						SetSelectedEntity(entity);
					}
		...
		if (*pOpenProperties)
		{
			...
			if (mSelectionEntity)
			{
				DrawComponents(mSelectionEntity);
			}
    ...
    void SceneHierarchyPanel::DrawEntityNode(Entity entity)
    {
        ...
        ImGuiTreeNodeFlags flags = ((mSelectionEntity == entity) ? ImGuiTreeNodeFlags_Selected : 0) | ImGuiTreeNodeFlags_OpenOnArrow;
	...
	{
		if (!mSelectionEntity.HasComponent<componentType>())
		{
			if (ImGui::BeginMenu(menuName))
			{
				if (ImGui::MenuItem(menuItemName))
				{
					mSelectionEntity.AddComponent<componentType>();
    ...
    void SceneHierarchyPanel::DrawComponents(Entity entity)
    {
        ...
        if (ImGui::BeginPopup("AddComponent"))
        {
			...
			if (ModeManager::b3DMode)
			{
				...
				//if (mSelectionEntity.HasComponent<MeshComponent>())
			...
			else
			{
				...
				//if (mSelectionEntity.HasComponent<SpriteRendererComponent>() || mSelectionEntity.HasComponent<CircleRendererComponent>())
				//{
				...
				if (ImGui::BeginDragDropTarget())
				{
					if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("ASSETS_BROWSER_ITEM"))
					{
						...
						if (texture->IsLoaded())
							...
							JENGINE_WARN("Could not load texture {0}", texturePath.filename().string());
					}
		...
		DrawComponent<MeshComponent>("Mesh Renderer", entity, [](MeshComponent& component)
			{
				...
				if (ImGui::Button("..."))
				{
					...
					{
						JENGINE_CORE_WARN("File path is empty");
						// TODO: Import Mesh
