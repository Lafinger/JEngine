...
namespace JEngine
{
	...
	void ContentBrowserPanel::OnImGuiRender(bool* pOpen)
	{
		...
		if (!ImGui::Begin("Assets Browser", pOpen))
		{
			ImGui::End();
			return;
		}
		...
		if (ImGui::BeginChild("ASSETS_TREE"))
		{
			DrawTree();
		}
		ImGui::EndChild();
		...
		if (ImGui::BeginChild("ASSETS_CONTENT"))
		{
			DrawContent();
		}
		ImGui::EndChild();
	...
	void ContentBrowserPanel::DrawContent()
	{
		...
		for (int i = 0; i < sortedDirectory.size(); i++)
		{
			...
			if (ImGui::BeginDragDropSource())
			{
				...
				ImGui::SetDragDropPayload("ASSETS_BROWSER_ITEM", itemPath, (wcslen(itemPath) + 1) * sizeof(wchar_t), ImGuiCond_Once);
