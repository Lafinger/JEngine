...
#include "Runtime/Application/Input/Input.h" // temp
...
#include "Runtime/Application/Utils/MathUtils.h"
...
namespace JEngine
{
    ...
    void EditorLayer::OnAttach()
    {
		...
		mEditorCamera = EditorCamera(30.0f, 1.778f, 0.1f, 10000.0f);
    ...
    void EditorLayer::OnUpdate(Timestep ts)
    {
		...
		if (selectedEntity)
		{
			...
			if (Input::IsKeyPressed(JENGINE_KEY_F))
    ...
    void EditorLayer::OnImGuiRender()
    {
		...
		if (bShowStats)
		{
			...
			ImGui::Text("Entities : %d", mActiveScene->mRegistry.size());

			uint32_t vertices_count = 0;
			uint32_t faces_count = 0;
			auto view = mActiveScene->mRegistry.view<MeshComponent>();
			for (auto entity : view)
			{
					auto& mesh_component = view.get<MeshComponent>(entity);
					for (auto submeshs : mesh_component.mMesh->mSubMeshes)
					{
							vertices_count += submeshs.mStaticVertices.size();
							faces_count += submeshs.GetFacesCount();
					}
			}
			ImGui::Text("Vertices : %d", vertices_count);
			ImGui::Text("Faces : %d", faces_count);

			std::string hovered_entity_name = "None";
			// if (mHoveredEntity) // has bug !!!
			// {
			// 	hovered_entity_name = mHoveredEntity.GetComponent<TagComponent>().Tag;
			// }
		...
		if (bShowEngineSettings)
		{
			...
			ImGui::NextColumn();
			ImGui::SliderFloat("##Camera Speed", &mEditorCamera.mCameraSpeed, 0.1f, 100.0f);
			ImGui::EndColumns();

			// ImGui::Checkbox("Debug Mode", &ModeManager::bDebugMode);
			...
			if (ImGuiWrapper::TreeNodeExStyle1((void *)"Post Processing", "Post Processing"))
			{
				if (ImGui::BeginMenu("Add Post Processing"))
				{
					constexpr auto postTypes = magic_enum::enum_values<PostProcessingType>();
		...
		if (bShowViewport)
		{
			...
			// Viewport is an image from framebuffer
			...
			// Put file to scene from content browser
			if (ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("ASSETS_BROWSER_ITEM"))
				{
					...
					JENGINE_CORE_INFO(std::filesystem::path(ConfigManager::GetInstance().GetAssetsFolder()) / path);

					std::filesystem::path file_path = std::filesystem::path(std::filesystem::path(ConfigManager::GetInstance().GetAssetsFolder()) / path);
					...
					{
							Entity entity1 = mActiveScene->CreateEntity(file_path.filename().string());
							entity1.AddComponent<MeshComponent>();
							auto& component = entity1.GetComponent<MeshComponent>();
			...
			// // scale, rotate, translate Toolbar
			...
			if (ImGui::Begin("Example: Simple overlay", &bShowSRT, windowFlags))
			{
				ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 100);
				{
					...
					if (ImGui::Button((const char*)ICON_MDI_CURSOR_DEFAULT))
					{
				...
				if (!mViewprotCollapsed)
				{
					...
					ImGui::SameLine();
					{
						...
						if (selected)
							ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.28f, 0.56f, 0.9f, 1.0f));
						if (ImGui::Button((const char*)ICON_MDI_ARROW_ALL))
						{
							clicked = true;
					...
					ImGui::SameLine();
					{
						...
						if (ImGui::Button((const char*)ICON_MDI_ROTATE_ORBIT))
						{
							clicked = true;
					...
					ImGui::SameLine();
					{
						...
						if (selected)
							ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.28f, 0.56f, 0.9f, 1.0f));
						if (ImGui::Button((const char*)ICON_MDI_ARROW_EXPAND_ALL))
						{
							clicked = true;
					...
					ImGui::SameLine();
					{
						...
						if (ImGui::Button((const char*)ICON_MDI_BORDER_NONE))
						{
							clicked = true;
					...
					// bool  ortho  = camera->isOrthographic();
					{
						...
						if (ImGui::Button((const char*)ICON_MDI_AXIS_ARROW " 3D"))
						{
							clicked = true;
							// if (ortho)
							// {
					...
					ImGui::SameLine();
					{
						if (selected)
							ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1, 0.4, 0.4, 1.f));
						if (ImGui::Button((const char*)ICON_MDI_ANGLE_RIGHT "2D"))
						{
							clicked = true;
							// if (!ortho)
							// {
					...
					// editor.drawPlayButtons();
					{
						...
						ImGui::SameLine();
						{
							...
							if (ImGui::Button(selected ? (const char*)ICON_MDI_STOP : (const char*)ICON_MDI_PLAY))
							{
						...
						ImGui::SameLine();
						{
							...
							if (ImGui::Button((const char*)ICON_MDI_PAUSE))
						...
						ImGui::SameLine();
						{
							...
							if (ImGui::Button((const char*)ICON_MDI_STEP_FORWARD))
					...
					// collapsed icon
					ImGui::SameLine();
					if (ImGui::Button((const char*)ICON_MDI_ARROW_LEFT))
					{
						mViewprotCollapsed = true;
					}
				}
				else
				{
					...
					ImGui::SameLine();
					if (ImGui::Button((const char*)ICON_MDI_ARROW_RIGHT))
					{
						mViewprotCollapsed = false;
					}
				}
			...
			glm::quat view_quat  = glm::identity<glm::quat>();
			view_quat = glm::quat_cast(mEditorCamera.GetViewMatrix());

			if (ImGui::Begin("Controls", &bShowSRT, windowFlags))
			{
				ImGui::gizmo3D("##gizmo1", view_quat, 96, imguiGizmo::sphereAtOrigin | imguiGizmo::modeFullAxes);
			}
			ImGui::End();
			// -----------------------------------------------------------toolbar------------------------------------------------------------------------------------
	...
	void EditorLayer::LoadDefaultEditorConfig()
	{
		...
		// JENGINE_CORE_ASSERT(std::filesystem::exists(DefaultEditorConfigPath));
    ...
    void EditorLayer::OnEvent(Event& e)
    {
        ...
        dispatcher.Dispatch<KeyPressedEvent>(JENGINE_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
        dispatcher.Dispatch<MouseButtonPressedEvent>(JENGINE_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
    }
