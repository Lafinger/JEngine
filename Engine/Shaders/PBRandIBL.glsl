...
float ShadowCalculation(vec3 fragPosWorldSpace)
{
    ...
    vec4 fragPosViewSpace = view * vec4(fragPosWorldSpace, 1.0); // view ?
    float depthValue = abs(fragPosViewSpace.z); // is not 0~1 ?
    ...
    vec4 fragPosLightSpace = lightSpaceMatrices[layer] * vec4(fragPosWorldSpace, 1.0); // lightSpaceMatrices ?
...
vec3 CookTorrance(vec3 n, vec3 l, vec3 v, float roughness, float metalness, vec3 f0, out vec3 kS)
{
    ...
    return (D * G * F) / (4.0 * max(NdotV * NdotL, 0.001f));
}
...
{		
    ...
    vec3 kS = F; // reflected part of the energy
    vec3 kD = 1.0 - kS; // remaining energy
    kD *= 1.0 - metallic; // if object is metallic, the remaining energy is absorbed
    ...
    // based_ambient is custom
    vec3 ambient = (vec3(based_ambient) * albedo + kD * diffuse + specular) * ao; // custom ambient + IBL
